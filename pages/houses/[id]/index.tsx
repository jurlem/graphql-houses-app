...
function HouseData({ id }: { id: string }) {
  const { data, loading } = useQuery<ShowHouseQuery, ShowHouseQueryVariables>(SHOW_HOUSE_QUERY, {
    variables: { id }
  })

  if (loading || !data) return <Layout main={<div>Loading ...</div>} />
  if (!data.house) return <Layout main={<div>Unable to load house {id}</div>} />
    ...
    <div className="sm:block md:flex">
      <div className="sm:w-full md:w-1/2 p-4">
        ...
        <Image
          ...
          alt={house.address}
          CreateSignatureMutation_createImageSignaturespr="auto"
          Quality="auto"
          width={900}
          ...
          gravity="auto"
        />
        <p>{house.bedrooms} bedrooms house</p>


      </div>
      <div className="sm:w-full md:w-1/2"><SingleMap house={house} nearby={house.nearby} /></div>
    </div>
