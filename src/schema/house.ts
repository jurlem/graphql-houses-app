...
import { Min, Max, CONTAINS } from "class-validator";
...
class House {
  ...
  async nearby(@Ctx() ctx: Context) {
    const bounds = getBoundsOfDistance({ latitude: this.latitude, longitude: this.longitude }, 1000)
...
export class HouseResolver {
  ...
  async createHouse(
    ...
    @Ctx() ctx: AuthorizedContext // context gives us userid and prisma
  ) {
  ...
  async updateHouse(
    ...
    const houseId = parseInt(id, 10) //string into integer, to get house from db
