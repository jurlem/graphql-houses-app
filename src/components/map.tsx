...
export default function Map({ setDataBounds, houses, highlightedId }: IProps) {
  ...
  // bounding box coordinates - what part of the world is showing up on the map
  ...
  return (<div className="text-black relative">
    ...
    >
      <div className="absolute top-0 w-full x-10 p-4">
        <SearchBox
          defaultValue=""
          onSelectAddress={(_address, latitude, longitude) => {
            if (latitude && longitude) {
              setViewport((old) => ({ ...old, latitude, longitude, zoom: 13 }))
            }
            if (mapRef.current) {
              const bounds = mapRef.current.getMap().getBounds()
              setDataBounds(JSON.stringify(bounds.toArray()))
      ...
      {houses.map(house => (
        ...
        >
          <button type="button" onClick={() => setSelected(house)}>
            <img src={
              highlightedId === house.id
                ? "/home-color.svg"
                : "/home-solid.svg"} className="w-8" alt="house" />
          </button>
        </Marker>
      ))}
      {selected && (
