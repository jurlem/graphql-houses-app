...
export default function HouseForm({ house }: IProps) {
  ...
  const [createHouse] = useMutation<CreateHouseMutation, CreateHouseMutationVariables>(CREATE_HOUSE_MUTATION)
  const [updateHouse] = useMutation<UpdateHouseMutation, UpdateHouseMutationVariables>(UPDATE_HOUSE_MUTATION)
  ...
  const handleUpdate = async (currentHouse: IHouse, data: IFormData) => {
    ...
    if (data?.image[0]) {
      ...
      if (signatureData) {
        const { signature, timestamp } = signatureData.createImageSignature
        const imageData = await uploadImage(data.image[0], signature, timestamp)
        //we replace what was in the current image url with updated
        image = imageData.secure_url
      }
    }
  ...
  const onSubmit = (data: IFormData) => {
    setSubmitting(true)
    // turning variable into boolean with !!
    if (!!house) {
  ...
  return (
    <form className="mx-auto max-w-xl py-4" onSubmit={handleSubmit(onSubmit)}>
      <h4 className="text-xl">{house ? `Editing ${house.address}` : "Add a New House"}</h4>

      <div className="mt-t">
        <label htmlFor="search" className="block pt-4">
      ...
      {address && (
        <>
          <div className="mt-4">
            <label htmlFor="image" className="p-4 border-dashed border-4 border-grey-600 block cursor-pointer">
          ...
          <div className="mt-4">
            ...
            <input id="bedrooms" name="bedrooms" type="number" className="p-2"
              ref={register({
                required: "Please enter the number if bedrooms",
                max: { value: 10, message: "Too big of a house" },
                min: { value: 1, message: "must be at leas 1" }
              })}
            />
          ...
          <div className="mt-4">
            ...
            <Link href={house ? `/houses/${house.id}` : "/"}><a>Cancel</a></Link>
          </div>
        </>
      )}
    </form>
